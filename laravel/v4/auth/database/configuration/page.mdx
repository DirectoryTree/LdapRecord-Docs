...
[alternate configured connection](/docs/core/v4/models#connections) name
...
The `model` option must be the class name of your [LdapRecord model](/docs/core/v4/models). This model will be used
...
The `scopes` option must be an array of [LdapRecord scope](/docs/core/v4/model-scopes.md) class names.
...
<Note>
We're using [automatic base DN subsitution](/docs/core/v4/searching#automatic-base-dn-substitution)
...
The `database => model` key is the class name of the [Eloquent model](https://laravel.com/docs/laravel/v4/eloquent) that will be
used for creating and retrieving LDAP users from your applications database.

<Note>
Be sure to add the required [trait and interface](/docs/laravel/v4/auth/database/installation#add-required-trait-and-interface) to this model as shown in the installation guide.
</Note>
...
<Important>
If the LDAP attribute returns `null` for the given **value**,
...
being [imported](/docs/laravel/v4/auth/database/importing), you can create an attribute handler class
