...
<Important>
This method is excellent for keeping memory usage
...
<Important>
Your LDAP server must support [Virtual List View](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/ldap/searching-with-the-ldap-vlv-control).
</Important>
...
<Important>
If you expect to have more than 1000 entries
...
Get the filter that has been added to the query:
...
// Returns LdapRecord\Query\Filter\Equals or null:
$filter = $query->getFilter();

$filter->getAttribute(); // "company"
$filter->getOperator(); // "="
$filter->getValue(); // "Acme"
(string) $filter; // "(company=Acme)"
```

When multiple conditions are added, the filter will be a group filter:
...
// Returns LdapRecord\Query\Filter\AndGroup:
$filter = $query->getFilter();

$filter->getOperator(); // "&"
$filter->getFilters(); // [Equals, Equals]
(string) $filter; // "(&(company=Acme)(department=Sales))"
```
...
<Important>
`objectclass` will always be included in the returned array.
</Important>
...
<Important>
This affects how the query filter is generated.
</Important>
...
<Important>
If only a single "or" is added to the query with no
...
<Important>
By selecting only the attributes you need, you can
