...
<Important>
Attributes that are set below can be cased in _any_ manor. They
...
<Important>
It is wise to encapsulate saving your user in a try / catch
...
[mutator](/docs/core/v4/model-mutators) has been added that assists in the setting
...
<Important>
You must use a try / catch block upon saving. An
...
A users `accountExpires` attribute stores a date (in [Windows Integer Time](/docs/core/v4/model-mutators/#windows-integer-type)) indicating when the account will no longer valid.

This attribute is already added as a [`windows-int` date cast](/docs/core/v4/model-mutators#date-mutators) inside of the included `ActiveDirectory\User` model.
...
This is called a [recursive relationship query](/docs/core/v4/model-relationships/#recursive-queries).
